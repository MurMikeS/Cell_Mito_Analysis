...
def Cell_Mask_Generate(cell_mask, cell_distance_set, element_region_size, cell_mask_ORG, cell_mask_contour,advanced_seg, save_sign, save_path, Sample_name):
    ...
    dist_transform = cv2.distanceTransform(th_cell_mask, cv2.DIST_L2, 5)
    localMax = peak_local_max(dist_transform, min_distance=cell_distance_set, labels=th_cell_mask)
    localMax_mask = np.zeros(dist_transform.shape, dtype=bool)
    localMax_mask[tuple(localMax.T)] = True
    Cell_markers = ndimage.label(localMax_mask, structure=np.ones((3, 3)))[0]
    Cell_labels = watershed(-dist_transform, Cell_markers, mask=th_cell_mask)
...
def main(all_data_path, **kwargs):
    ...
    for s_i in range(len(Sample_list)):
        ...
        assert cell_mask is not None, "file could not be read, check with os.path.exists()"
        ...
        assert mos_mask is not None, "file could not be read, check with os.path.exists()"
